...
<script>
	...
	function initFn(){
		...
		var tableStructure = {
			...
			,{display : ' 成功率 ', code : 'successRate', width : 100, align : 'left', type:'fun',value:function(obj){
				var ls="";
				if(obj.total>0){
					ls=obj.successNum/obj.total;
					ls=Math.round(ls*10000);
					ls=ls/100+"%";
				}else{
					ls=0+"%";
				}				
			...
			,{display : ' 失败率 ', code : 'failRate', width : 100, align : 'left', type:'fun',value:function(obj){
				var ls="";
				if(obj.total>0){
					ls=obj.failNum/obj.total;
					ls=Math.round(ls*10000);
					ls=ls/100+"%";
				}else{
					ls=0+"%";
			...
			,{display : ' 故障率 ', code : 'errorRate', width : 100, align : 'left', type:'fun',value:function(obj){
				var ls="";
				if(obj.total>0){
					ls=obj.errorNum/obj.total;
					ls=Math.round(ls*10000);
					ls=ls/100+"%";
				}else{
					ls=0+"%";
			...
			,{display : ' 数据成功返回率 ', code : 'successRelRate', width : 100, align : 'left', type:'fun',value:function(obj){
				var ls="";
				if(obj.total>0){
					ls=obj.successRelNum/obj.total;
					ls=Math.round(ls*10000);
					ls=ls/100+"%";
				}else{
					ls=0+"%";
			...
			url : "${basePath}extInterfaceLog/queryExtInterfaceLogStatus?parentId=1&successStatus=${successStatus}&failStatus=${failStatus}&errorStatus=${errorStatus}&successRelStatus=${successRelStatus}",
