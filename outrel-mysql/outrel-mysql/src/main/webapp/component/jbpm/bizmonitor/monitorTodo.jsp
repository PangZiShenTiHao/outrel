...
<%@ page language="java" import="com.jy.modules.befloan.jbpm.tool.ObtainBizRuleConfTool,com.jy.platform.api.org.UserInfo,com.jy.platform.api.org.SessionAPI,org.springframework.web.context.support.WebApplicationContextUtils,org.springframework.web.context.WebApplicationContext,org.springframework.web.context.ContextLoader,org.springframework.context.ApplicationContext" %>
...
<body style="background-color:#FFFFFF">
	...
	<%--
	String roles = (String)pageContext.getAttribute("roles");
	String roleFilters = ObtainBizRuleConfTool.makeupRoleFilters(roles);
	--%>
<!-- 列表按钮操作 start -->
	...
	function initFn(){
		...
		//定义 form表单查询 信息
		 var formStructure={
	        ...
	        <%-- ,{display : '姓名 ', code : 'processParticipationName', width : 200,  type:'autocomplete',value:<sysuser:search type="json" hasBlank="true" emptyText="" roleCodes="<%=roleFilters%>" />}--%>
	        ...
	        ,{display : '任务到达时间', code : 'time', width : 100, align : 'left',type: 'dbDateTime',isCompare:true}
		...
		var tableStructure = {
				...
				{display : '任务名称', code : 'BIZ_INF_NAME', width : 130, align : 'left', type:'fun',
					value:function (obj){
						...
						v_aInfo = v_aInfo+"</a>";
						*/
						if(""!=obj.EXT2&& typeof(obj.EXT2) != "undefined"){
	  						v_aInfo = v_aInfo + _font+ " " +obj.EXT2;
...
// 									 mult = "";
// 								 }
// 								 v_aInfo = v_aInfo+"<a class='repeat' title='' onMouseOver='repeatView(\""+obj.BIZ_INF_NAME+"\",\""+msg.length+"\")' onclick='doRepeatView(\""+obj.BIZ_INF_NAME+"\",\""+msg.length+"\",\""+msg[0]["ID"]+"\")' id=\""+obj.BIZ_INF_NAME+"\" href='javascript:void(0)'>"+mult+"</a>";
// 							 }/*else{
// 								 v_aInfo = v_aInfo+"</a>";
// 							 }*/
// 							}
// 						});
				...
				{display : ' 流程状态', code : 'PRO_INSTANCE_STATE', width : 60, align : 'center', type:'select',value:[{value:'1',text:'正常'},{value:'0',text:'挂起(暂停)'}]},
			...
			url : "${basePath1}dojbpm/monitorjbpmbiz/findTaskInfo.do?customSQL=Y&extSort=ext1,",
			pageSize : 30,
