...
debugger;
	...
	//获取 弹出页面 querySelectPartner 的 selectedPartner 隐藏域内容
	//参与者 用户名称
	//var v_userName=$(window.parent.document).find("[name='selectPartnerDialogId']").contents().find("[id='selectedPartnerRealName']").val();
	//参与者user ID
	//var v_userId=$(window.parent.document).find("[name='selectPartnerDialogId']").contents().find("[id='selectedPartner']").val();
	var v_userName=$('#selectedPartnerRealName').val();
	var v_userId=$('#selectedPartner').val();
	...
	try{
		...
		isIframe = isIframe.substring(0,isIframe.indexOf("?"));
		if(isIframe){
			window.parent.setPartnerInfo(v_userName,v_userId);
		}else{
