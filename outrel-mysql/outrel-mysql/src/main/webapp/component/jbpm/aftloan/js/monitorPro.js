...
function batchUpdateAssignee(){
	...
	//选中的待办任务对象
	var v_selectedObjs = iframe.iframeObj["table"].getSelectedObjs();
	//选中的待办任务对象数组
	var v_tasksArray = [];
	for(var i=0;i<v_selectedObjs.length;i++){
		var v_selectedObj = v_selectedObjs[i];
		var v_taskObj = {};
		v_taskObj.proInstanceId = v_selectedObj.CUR_EXE_ID;//流程实例ID
		v_taskObj.bizInfId = v_selectedObj.BIZ_INF_ID;//进件编号（任务名称）
		v_taskObj.activeName = v_selectedObj.CUR_ACT_NAME;//节点名称
		v_taskObj.ownerId = v_selectedObj.START_PRO_USERID;//业务归属人
		v_taskObj.fkJbpmBizTabId = v_selectedObj.BIZ_TAB_ID;//工作流与业务表关联表 JBPM4_BIZ_TAB主键ID
		v_taskObj.remark='换人';
		v_tasksArray.push(v_taskObj);
	}
	//为了页面传值，将选中的对象数组转化为Json字符串
	var v_tasksJson = JSON.stringify(v_tasksArray);
	$.dialog({
	    ...
	    height:860,
	    title:'转移任务',
	    content: 'url:'+contextRootPath+'/component/jbpm/updateAssignee.jsp?tasks='+v_tasks+'&tasksJson='+v_tasksJson
	 });
}
/**
