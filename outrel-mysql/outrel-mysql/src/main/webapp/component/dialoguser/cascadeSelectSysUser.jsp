...
function resetData(){
	...
	var url1=contextRootPath+'/lbTCustomterInfo/getChildNodes?a=a&orgCode=301';
	var url2=contextRootPath+'/lbTCustomterInfo/getStores?a=a&orgCode=301';
	var url3=contextRootPath+'/lbTCustomterInfo/getAllStores?a=a&1=1';
	$("input[name='areaCode']").newCascadeObject(2,"areaCode:" + url1,"branchCode:" + url2,"orgId:" + url3);
}
...
function initFn (){
	//定义 form表单查询 信息
	 var formStructure={
        ...
        <c:if test="${'select' ne param.orgSelectType}">
        ,{display : '机构/部门ID', code : 'orgId', width : 200,  type:'hidden'}
        ,{display : '机构/部门', code : 'orgDeptName', width : 200,  type:'text'}
        </c:if>
        <c:if test="${'select' eq param.orgSelectType}">
        ,{display : ' 大区', code : 'areaCode', width : 200,  type:'text'}
        ,{display : ' 分部', code : 'branchCode', width : 200,  type:'text'}
		,{display : ' 营业部', code : 'orgId', width : 200,  type:'text'}
        </c:if>
		],
		//定义form 表单 按钮信息
		buttons:[
...
function doSure(){
	...
	if("true" == $("#check").val()){
		select_sys_user.each(function(v_key,v_val){
			if(v_oldIds){
				v_oldIds +=",";
				v_oldNames +=",";
			}
...
$(document).ready(function(){
	...
	if(orgId == null || orgId =="" || orgId == 'null' || orgId == '0')orgId="-1";//生产环境 捷越联合 orgCode及ID是6
	...
	$("#selectOrgId").val(orgId);
   	...
   	<c:if test="${'select' eq param.orgSelectType}">
	   /* 	var  orgurl=contextRootPath+'/sysOrg/queryTreeSysOrg?a=a&parentId=301';
		$("input[name='areaCode']").newCascadeObject(1,2,"areaCode:" + orgurl,"branchCode","orgId"); */
		//大区
		var url1=contextRootPath+'/lbTCustomterInfo/getChildNodes?a=a&orgCode=301';
		var url2=contextRootPath+'/lbTCustomterInfo/getStores?a=a&orgCode=301';
		var url3=contextRootPath+'/lbTCustomterInfo/getAllStores?a=a&1=1';
		...
		//分部
		var url2=contextRootPath+'/lbTCustomterInfo/getStores?a=a&orgCode=301';
		$("input[name='branchCode']").newCascadeObject(2,2,"branchCode:" + url2,"","",{methodName:"methodName",methodFirstText:"${custInfo.nation }"});
		
		//营业部
		var url3=contextRootPath+'/lbTCustomterInfo/getAllStores?a=a&1=1';
		$("input[name='orgId']").newCascadeObject(2,2,"orgId:" + url3,"","",{methodName:"methodName",methodFirstText:"${custInfo.nation }"}); */
   	</c:if>
   	queryData();//查询
});
</script>
