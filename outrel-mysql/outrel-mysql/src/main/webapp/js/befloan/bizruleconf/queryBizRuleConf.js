//utf-8
//点击新增按钮 事件跳转至 新增页面
var dialogAddObj;
var dialogUpdateObj;
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/bizRuleConf/prepareExecute/toAdd',
		'width': 450,
		'height':350,
		'title':'新增',
		'buttons':[
         {'text':'保存','action':doAddFrom,'isClose':false},
		 {'text':'关闭','isClose':true}
		]
	};
	
	dialogAddObj =jyDialog(dialogStruct);
	dialogAddObj.open();
}
//新增页面的保存操作
function doAddFrom(){
	debugger;
	var obj = dialogAddObj.getIframe();
	obj.saveMyData();
	var flag = checkIsNull(obj.$("#addNewsFormData")[0]);
	if(flag){
		//序列化 新增页面的form表单数据
		var params=obj.$("#addNewsFormData").serialize();
		//return ;
		var url=contextRootPath+'/bizRuleConf/insertBizRuleConf';
		//通过ajax保存
		jyAjax(
			url,
			params,
			function(msg){
				//新增成功后，
				//alert(msg.msg);
				$(window.top.body).newMsg({}).show(msg.msg);
				var v_status = msg.status;
	        	if(v_status.indexOf('ok') >-1){
	        		dialogAddObj.close();
	        		//新增成功后 刷新页面 或 只查询 id为msg.data['id'] 的  数据
	        		queryData();
	        	}
	  	});
	}
}
//修改 事件
function toUpdateData(){
	var v_ids = iframe.iframeObj["table"].getSelectedIds().join(",");
	//如果没有选中 数据则
	if(v_ids == ""){
		alert("请选择待修改的数据！");
		return;
	}
	//如果选择多个 择提示
	if(v_ids.indexOf(",") > -1){
		alert("请选择一条数据！");
		return;
	}

	var dialogStruct={
			'display':contextRootPath+'/bizRuleConf/prepareExecute/toUpdate?id='+v_ids,
			'width':450,
			'height':350,
			'title':'修改',
			'buttons':[
             {'text':'保存','action':doUpdateFrom,'isClose':false},
			 {'text':'关闭','isClose':true}
			]
		};
		
	dialogUpdateObj = jyDialog(dialogStruct);
	dialogUpdateObj.open();
}
//修改页面 的 保存操作
function doUpdateFrom(){
	var obj = dialogUpdateObj.getIframe();
	var flag = checkIsNull(obj.$("#updateNewsFormData")[0]);
	if(flag){
		var params=obj.$("#updateNewsFormData").serialize();
		var url=contextRootPath+'/bizRuleConf/updateBizRuleConf';
		//通过ajax保存
		jyAjax(
			url,
			params,
			function(msg){				
				$(window.top.body).newMsg({}).show(msg.msg);
	        	var v_status = msg.status;
	        	if(v_status.indexOf('ok') >-1){
	        		//dialogUpdateObj.close();
	        		//新增成功后 刷新页面 或 只查询 id为msg.data['id'] 的  数据
	        		//queryData();
	        	}
	  	});
	}
}
//删除 事件
function deleteData(){
	var v_ids = iframe.iframeObj["table"].getSelectedIds().join(",");
	//如果没有选中 数据则
	if(v_ids == ""){
		alert("请选择待删除的数据！");
		return;
	}
	if(confirm("您确认要删除选择的数据吗？")){
        $.ajax({
            type:"POST",
            url:contextRootPath+"/bizRuleConf/deleteBizRuleConf?ids="+v_ids,
            success:function(msg){
            	var msg = eval('('+msg+')');
            	alert(msg.msg);
            	var v_status = msg.status;
            	//删除成功后
            	if(v_status.indexOf('ok') >-1){
            		iframe.iframeObj["table"].removeSelectedRow();
            	}
            	
            	
            }
        });
    }
}
//查看
function viewConfInf(vId){
	var dialogStruct={
			'display':contextRootPath+'/bizRuleConf/prepareExecute/toView?id='+vId,
			'width':380,
			'height':400,
			'title':'预览',
			'buttons':[
			 {'text':'关闭','isClose':true}
			]
	};
		
	var dialogView = jyDialog(dialogStruct).open();
	viewDataDetailById(vId);
}

/*
 *规则详细
 */
function viewDataDetailById(vId){
	$(parent.document).find("#right").attr("src",contextRootPath+"/bizRuleConf/prepareExecute/toQueryList?fatherId="+vId);
}

/*
 *规则详细
 */
function viewDataDetail(){
	var vId = iframe.iframeObj["table"].getSelectedIds().join(",");
	viewDataDetailById(vId);
}
