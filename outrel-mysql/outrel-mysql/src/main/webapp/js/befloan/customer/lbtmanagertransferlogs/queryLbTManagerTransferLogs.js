...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/lbTManagerTransferLogs/prepareExecute/toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'buttons':[
...
function doAddFrom(){
	...
	var url=contextRootPath+'/lbTManagerTransferLogs/insertLbTManagerTransferLogs';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/lbTManagerTransferLogs/prepareExecute/toUpdate?id='+v_ids,
			'width':800,
			'height':500,
			'title':'修改',
			'buttons':[
...
function doUpdateFrom(){
	...
	var url=contextRootPath+'/lbTManagerTransferLogs/updateLbTManagerTransferLogs';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	if(confirm("您确认要删除选择的数据吗？")){
        $.ajax({
            type:"POST",
            dataType:"JSON",
            url:contextRootPath+"/lbTManagerTransferLogs/deleteLbTManagerTransferLogs?ids="+v_ids,
