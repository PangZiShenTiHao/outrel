// new表格
function TreeMenuLoan(treeMenu,treeObj) {
	debugger;
...
TreeMenuLoan.prototype = {
	init:function(){
            ...
            this.valueChange=this.treeMenu.valueChange;
            this.callBackLoan=this.treeMenu.callBackLoan;
		...
		var treeDiv=$("#"+that.treeId);
		 if(!(treeDiv&&treeDiv.length)){
			 ...
			 var createBtn=function(bit){
				 var buttons=[];
				 var aStyle=' display:inline-block; margin: 2px 2px; text-decoration: none; color: #0A5991; font-weight: bold;';
				 if(bit){
			 ...
			 var createTree=function(bit){
				 ...
				 $("body").append('<div class="ui-widget-content treeMenu" style="left:'+(left+14)+'px; top:'+(top+10)+'px;width:'+that.width+'px;height:'+that.height+'px;" id="'+that.treeId+'">'+cDiv+btnDiv+'</div>');
				 btnOp.addEvent();
			 };
				...
				if(this.treeType=="radio"){
					...
					ownTreeS["viewFun"]=function(nodeId,node){
                        ...
                        else
                        {
							...
							that.recovery(that.valueChange,that.callBackLoan);
                        }
	...
	recovery:function(valueChange,callBackLoan){
		if(valueChange){
			eval(callBackLoan);
		}
		var that=this;
...
(function( $ ){  
  $.fn.treeMenuLoan = function(menuTreeStructure) {
   	 return new TreeMenuLoan(menuTreeStructure,$(this)[0]);
  };
})( jQuery );  
