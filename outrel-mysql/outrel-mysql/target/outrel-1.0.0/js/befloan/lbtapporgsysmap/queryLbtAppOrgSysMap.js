...
function toAddData(){
	var dialogStruct={
		'display':contextRootPath+'/lbtAppOrgSysMap/prepareExecute/toAdd',
		'width':800,
		'height':500,
		'title':'新增',
		'buttons':[
...
function doAddFrom(){
	...
	if(!checkIsNull(obj.$("#addNewsFormData"))) {
		$("").newMsg({}).show("请输入相关必输项！");
		
	}else{
		var params=obj.$("#addNewsFormData").serialize();
		var url=contextRootPath+'/lbtAppOrgSysMap/insertLbtAppOrgSysMap';
		//通过ajax保存
		jyAjax(
			url,
			params,
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/lbtAppOrgSysMap/prepareExecute/toUpdate?id='+v_ids,
			'width':800,
			'height':300,
			'title':'修改',
			'buttons':[
...
function doUpdateFrom(){
	...
	var url=contextRootPath+'/lbtAppOrgSysMap/updateLbtAppOrgSysMap';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	jyDialog().confirm("您确认要删除选择的数据吗？", function(){
		var url = contextRootPath+'/lbtAppOrgSysMap/deleteLbtAppOrgSysMap?ids='+v_ids;
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/lbtAppOrgSysMap/prepareExecute/toView?id='+vId,
			'width':800,
			'height':500,
			'title':'查看明细',
			'buttons':[
