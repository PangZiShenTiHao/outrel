...
function toAddData(){
	var dialogStruct={
			'display':contextRootPath+'/queryLoanInfoDate/prepareExecute/toQueryBefloanForLoanInfo?function=Gps&statusName=penalty',
			'width':1100,
			'height':750,
			'title':'查询贷款信息',
			'buttons':[
...
function openNextPage(){
	...
	addDlg1.close();
	//打开新增界面
	var url=contextRootPath+'/laTPenalty/prepareExecute/toAddNext?'+selectObj+'&claimKind='+claimKind;
	var dialogStruct={
			'display':url,
			'width':1100,
			'height':750,
...
function doAddFrom(){
	...
	if(checkTrueCount && checkInto(formObj)) {
 	...
 	var url=contextRootPath+'/laTPenalty/insertLaTPenalty';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function toUpdateData(){
	...
	var dialogStruct={
			'display':contextRootPath+'/laTPenalty/prepareExecute/toUpdate?id='+v_ids,
			'width':1100,
			'height':750,
			'title':'修改罚制扣款信息',
			'buttons':[
...
function doUpdateFrom(){
  ...
  if(checkTrueCount && checkInto(formObj)) {
	...
	var url=contextRootPath+'/laTPenalty/updateLaTPenalty';
	//通过ajax保存
	jyAjax(
		url,
		params,
...
function deleteData(){
	...
	jyDialog({"type":"question"}).confirm("您确认要删除选择的数据吗？",function(){
		$.ajax({
            type:"POST",
            dataType:"JSON",
            url:contextRootPath+"/laTPenalty/deleteLaTPenalty?ids="+v_ids,
...
function viewData(vId){
	var dialogStruct={
			'display':contextRootPath+'/laTPenalty/prepareExecute/toView?id='+vId,
			'width':1100,
			'height':750,
			'title':'查看罚制扣款明细',
			'buttons':[
...
function toUploadFile(){
	...
	var dialogStruct={
			'display':contextRootPath+'/laTImageBizAss/prepareExecute/toViewUpload?id='+v_ids+"&type=21",
			'width':1000,
			'height':800,
			'title':'上传罚制扣款信息附件',
			'buttons':[
...
function submitData1(){
	var v_ids = iframe.iframeObj["table"].getSelectedIds().join(",");
	var objs = iframe.iframeObj["table"].getSelectedObjs();
	var url = contextRootPath+"/penaltyjbpm/jbpmPenalty/startPenaltyWorkflow?nextPartnerId="+$("#dtoparamUserId").val()+"&id="+v_ids+"&loginUserId="+$("#loginUserId").val();
	$.ajax({
	     type:"POST",
	     dataType:"JSON",
	     url:url,
...
function checkUpload(objs) {
	var isUpload = false;
  	var  url = contextRootPath+"/laTImageBizAss/checkUpload?listId="+objs[0]["listId"] + "&listType=21";
	$.ajax({
		type:"POST",
		url:url,
		async:false,
