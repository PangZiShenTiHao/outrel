	 ...
	 if(isUploadFlag!="NO"){
		 var treeCon = {
				...
				"editBtnExt":function(node){
				    ...
				    if(!(isUploadFlag=="NO")){
				    	if(node.UPLOAD=="Y"){
							...
							return [{name:"coverfile",fun:coverFile},{name:"downloadfile",fun:downloadFile}];
						}
				    }
				},
				"async": {
		  ...
		  var treeCon = {
					...
					"check" : false,
					"checkChildNodes" : false,
					"url" : contextRootPath+ '/lbTCustAttach/queryViewImagesAppendix?prodId='+ $("#prod_Id").val() + "&intoId="+ $("#into_Id").val() + "&custId="+ $("#cust_Id").val()+"&attType="+$("#attType").val(),
					"viewFun" : viewFuns,
					"groupData":function(obj){
						if(obj["ISMUST"]=='1'){
							obj["icon"]=contextRootPath+ "/js/plat/base/icon/important.png";
...
function queryAttachByType(){
	var url = contextRootPath+'/lbTCustAttach/queryViewImagesAppendixByType';
	var params = "prodId="+$("#prod_Id").val()+"&intoId="+$("#into_Id").val()+"&custId="+$("#cust_Id").val() +"&attType=" + $("#attType").val();
